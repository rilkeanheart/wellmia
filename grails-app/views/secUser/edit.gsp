...
<%
def tabData = []
tabData << [name: 'userinfo', icon: 'icon_user', messageCode: 'wellmia.security.ui.user.info']
tabData << [name: 'roles',    icon: 'icon_role', messageCode: 'wellmia.security.ui.user.roles']
boolean isOpenId = PluginManagerHolder.pluginManager.hasGrailsPlugin('springSecurityOpenid')
if (isOpenId) {
	tabData << [name: 'openIds', icon: 'icon_role', messageCode: 'wellmia.security.ui.user.openIds']
}
%>
...
<g:if test='${canRunAs}'>
<a id="runAsButton">${message(code:'wellmia.security.ui.runas.submit')}</a>
</g:if>
